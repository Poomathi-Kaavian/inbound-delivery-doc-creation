...
export function apiInboundDeliverySrv0002<
  BinaryT = string,
  BooleanT = boolean,
  ByteT = number,
  DecimalT = BigNumber,
...
>(
  ...
  >
> {
  return new ApiInboundDeliverySrv0002(
    mergeDefaultDeSerializersWith(deSerializers)
  );
}
class ApiInboundDeliverySrv0002<
...
> {
  ...
  get operations() {
    return {
      ...
      createBatchSplitItem: (
        parameter: CreateBatchSplitItemParameters<DeSerializersT>
      ) => createBatchSplitItem(parameter, this.deSerializers),
      addSerialNumberToDeliveryItem: (
