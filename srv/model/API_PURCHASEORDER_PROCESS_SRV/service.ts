...
export function apiPurchaseorderProcessSrv<
  BinaryT = string,
  BooleanT = boolean,
  ByteT = number,
  DecimalT = BigNumber,
...
>(
  ...
  >
> {
  return new ApiPurchaseorderProcessSrv(
    mergeDefaultDeSerializersWith(deSerializers)
  );
}
class ApiPurchaseorderProcessSrv<
...
> {
  ...
  get purchaseOrderItemApi(): PurchaseOrderItemApi<DeSerializersT> {
    ...
    const linkedApis = [
      ...
      this.initApi('valAddedSrvcMm_2Api', ValAddedSrvcMm_2Api)
    ];
    api._addNavigationProperties(linkedApis);
    return api;
  }
  ...
  get valAddedSrvcMm_2Api(): ValAddedSrvcMm_2Api<DeSerializersT> {
    return this.initApi('valAddedSrvcMm_2Api', ValAddedSrvcMm_2Api);
  }
